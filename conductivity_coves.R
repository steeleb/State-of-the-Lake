...
## map of 10yr conductivity mean ----
# we'll take the mean of the median annual value per site over the previous 10 years

lmp_cond_aggyearsite_cove_10yr <- lmp_cond_aggyearsite_cove %>% 
  filter(year >= 2014) %>% 
  group_by(station, site_type, sub_site_type, lat_dd, lon_dd) %>% 
  summarize(mean_cond_uScm = mean(mean_cond_uScm))
